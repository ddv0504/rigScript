...
class SatyrLeg(PrimaryRigPart, SwitchableMixin):
	...
	def _build( self, skeletonPart, stretchy=True, **kw ):
		...
		footTrigger.createMenu( 'switch to FK', '''python( "from ztZoo import rigPrimitives; rigPrimitives.RigPart.InitFromItem('#').switchToFk()" )''' )

		for c in fkControls:
			t = Trigger( c )
			t.createMenu( 'switch to IK', '''python( "from ztZoo import rigPrimitives; rigPrimitives.RigPart.InitFromItem('#').switchToIk()" )''' )
