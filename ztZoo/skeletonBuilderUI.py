...
class BuildPartLayout(MelFormLayout):
	...
	def getKwargDict( self ):
		kwargs = {}
		for arg, ui in self.kwarg_UIs.items():
		...
		return kwargs
	def rePopulate( self ):
		for argName, ui in self.kwarg_UIs.items():
...
class BuildingLayout(MelScrollLayout):
	...
	def rePopulatePresets( self ):
		...
		for locale, presets in skeletonBuilderPresets.listPresets().items():
...
class EditPartLayout(MelFormLayout):
	...
	def populate( self ):
		...
		for arg, argValue in buildKwargs.items():
	...
	def getBuildKwargs( self ):
		kwargs = {}
		for argName, widget in self.argUIs.items():
