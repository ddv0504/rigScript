...
class P4Output(dict):
	...
	def __init__( self, outStr, keysColonDelimited=False ):
		...
		for prefix, dataList in multiKeys.items():
			try:
				self.pop( prefix )
	...
	def asStr( self ):
		...
		return '\n'.join( '%s:  %s' % items for items in self.items() )
...
def populateChange( change ):
		...
		if isinstance( changeNum, int ) and changeNum:
			...
			for key, value in fullChange.items():
				change[ key ] = value
...
#gets called when a perforce command takes too int (defined by P4File.TIMEOUT_PERIOD)
...
class P4File(Path):
	...
	def setChange( self, newChange=None, f=None ):
		...
		if isinstance( newChange, (int, int) ):
			change = newChange
