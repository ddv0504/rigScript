...
def getDeps( aModule ):
	...
	def _getDeps( module ):
		...
		for n, o in aModule.__dict__.items():
			try:
...
class DependencyNode(dict):
	...
	def findDependents( self, changedScriptPath ):
		affected = []
		for srcFile, depNode in self.items():
...
class DependencyTree(DependencyNode):
	...
	def findDependents( self, changedScriptPath ):
		...
		for script, depNode in self.items():
		...
		if primaryAffected:
			...
			def gatherSecondaryDependencies( theDepNode ):
				stillAdding = False
				for script, depNode in theDepNode.items():
					if script == changedScriptPath:
						continue
					...
					if script in primaryAffected or script in secondaryAffected:
						...
						for sub_script in depNode.keys():
...
def getScriptTests( scriptFilepath, depTree=None ):
	...
	for test, testDependencies in testDependencyDict.items():
