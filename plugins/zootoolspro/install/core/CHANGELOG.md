...
**Added**
 - Added env command to embed zoo into current process.
 - running zoo_cmd with no arguments displays help instead of embedding environment.
 
 ## 1.1.21 (2020-11-07)
 
 **Added**
 - Support to turn off maya plugin autoload

 ## 1.2.0 (2020-12-22)
 
 **Changed**
 - Removal of pathlib2 and scandir thirdparty libraries.

 ## 1.2.1 (2020-12-29)
 
 **Changed**
 - Basic update to documentation formatting.
 - Qt.py updated to 1.3.3

 ## 1.2.2 (2021-04-05)
 
 **Bug Fixes**
- cmd fixing exit codes.
- py3 update fixes.
- fix linux shell command.
- fix zoo_cmd exitcode.

 **Changes**
- jsonlogger update to latest.
- six update to latest.
- support {install} token in package descriptor paths.
- fix *.pyc being included on package release and copying.

 ## 1.2.3 (2021-05-10)
 
 **Bug Fixes**
- maya 2022 startup security handling. 
- maya plugin unable to load without user setup script.
- fix package version config being empty as a fallback when not loading in maya.

 **Changes**
- Ported zooundo from zoo_maya
- ported libs.plugin(zoo_core).
- ported some required libraries from libs.utils(zoo_core).
- port libs.utils.env(zoo_core).
