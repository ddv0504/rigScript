...
from zoo.preferences import preferencesconstants as pc, prefutils
...
class MayaShaders(toolsetwidget.ToolsetWidget):
    id = "mayaShaders"
    uiData = {"label": "Maya Shaders",
    ...
    def preContentSetup(self):
        ...
        self.generalPrefs = prefutils.generalInterface()
        self.generalSettingsPrefsData = self.generalPrefs.settings()  # renderer in general pref
        self.properties.rendererIconMenu.value = self.generalPrefs.primaryRenderer()
        self.shaderPrefs = preference.interface("shader_interface")
        self.shaderToolsPrefsData = self.shaderPrefs.settings()  # shader .prefs info
    ...
    def updateFromProperties(self):
        ...
        """
        super(MayaShaders, self).updateFromProperties()
    ...
    def setPrefVariables(self):
        ...
        if not self.shaderToolsPrefsData.isValid():  # should be very rare
            ...
            return
        self.directory = self.shaderToolsPrefsData["settings"][sc.PREFS_KEY_MAYASHADERS]
        self.uniformIcons = self.shaderToolsPrefsData["settings"][sc.PREFS_KEY_MSHADERS_UNIFORM]
    ...
    def global_changeRenderer(self):
        ...
        self.generalSettingsPrefsData = self.generalPrefs.settings(refresh=True)  # refresh data
    ...
    def uniformIconToggle(self, action):
        ...
        self.shaderToolsPrefsData["settings"][sc.PREFS_KEY_MSHADERS_UNIFORM] = self.uniformIcons
    ...
    def filePath(self, message=True):
        ...
        :type message: bool
        :return filePaath: the full path of the file of the selected thumb, will be "" if none selected
        :rtype filePaath: str
        """
        try:
    ...
    def importScene(self, assetName=""):
        ...
        shaderutils.importAndAssignShader(path, mayaFileType=mayaType)
