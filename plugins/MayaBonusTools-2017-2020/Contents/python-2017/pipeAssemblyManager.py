 ...
 if os.path.exists(pipeDir):
    ...
    def PAMAssetPopup(*args):
       ...
       if cmds.textScrollList(txScrlLst,q=1,nsi=1) > 0:
            ...
            tempPath = tempPipePath + tempPipeShow + "/lib/assets/" + tempID + "/data/variations/initial"
    ...
    def loadPipeAssets(*args):
        ...
        if selected==None:
            ...
            for name in selected:
                ...
                tempPath = tempPipePath + tempPipeShow + "/lib/assets/" + tempID + "/data/definition/initial/" + name + ".ma"
...
# Get New Asset Info
    ...
    tempNewAssemblyBasePath = pipeBasePath + show + "/lib/assets/" + tempNewAssemblyID + "/data/"
    tempNewAssemblyRepBase = tempNewAssemblyBasePath + "variations/initial/"
    tempNewAssemblyDefPath = tempNewAssemblyBasePath + "definition/initial/" + newAssetName + ".ma"
# Create Geo Rep File
    grpName = ""
    ttt=  ""
    ttt2 = ""
...
# Create GPU Rep File
    if makeGPU==True:
        ...
        repPathLocal = "lib/assets/" +  tempNewAssemblyID + "/data/variations/initial/gpu/cache/" + newAssetName + ".initial.gpu.abc"
...
# Make BBox Rep
    if makeBBox==True:
        ...
        repPathLocal = "lib/assets/" +  tempNewAssemblyID + "/data/variations/initial/bBox/maya/" + newAssetName + ".initial.bBox.ma"
...
# Delete everything new and move the Originals back to where they belong
        ...
        cmds.setAttr(feedback + ".definition", ("lib/assets/" +  tempNewAssemblyID + "/data/definition/initial/" + newAssetName + ".ma"), type="string")
        cmds.select(feedback)
        cmds.move(relX,relY,relZ, feedback, a=True, ws=True)
        cmds.assembly(feedback,e=1, active="gpu")
        cmds.confirmDialog(message= 'Asset "' + newAssetName + '" Assemblified.\n', button=["OK"])
        PAMAssetEditInitialize()
...
# build definition save path
        ...
        tempPipeAssVarList=pipe.pipeListAssetVarReps(asset,'initial')
        tempID = pipe.pipeGetAssetInfoName(asset,"Id")
        tempPath = tempPipePath + tempPipeShow + "/lib/assets/" + tempID + "/data/definition/initial/" + asset + ".ma"
...
# create the file name and path
               ...
               tempRepPath = "lib/assets/" + tempID + '/data/variations/initial/' + tempRep + '/' + tempSubdir + '/'
...
# file node
                    ...
                    if cmds.pluginInfo("Substance",query=True, loaded=True):
                        ...
                        substanceLoc =  mayaLocation + "/substance/scripts/AEsubstanceTemplate.mel"
        ...
        if response == "Continue":
            ...
            if os.path.exists(repositoryPath) == True:
                if os.path.exists(repositoryPath + "/pipe") == True:
                    ...
                    os.makedirs(repositoryPath + "/pipe", 7777)
                if os.path.exists(os.environ['HOME']+"/pipe/")!=True:
                     os.makedirs(os.environ['HOME']+"/pipe/", 7777)
                file = open(os.environ['HOME']+"/pipe/pipe.cfg","w")
