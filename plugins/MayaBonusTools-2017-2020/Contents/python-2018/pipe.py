...
# Make Startup Directories
    ...
    if os.path.exists(pipeXMLConfigPath)!=True:
        os.makedirs(pipeXMLConfigPath, 7777)
    if os.path.exists(pipeAssetLibPath)!=True:
        os.makedirs(pipeAssetLibPath, 7777)
        os.makedirs(pipeAssetLibPath+"assets/", 7777)
        os.makedirs(pipeAssetLibPath+"xml/", 7777)
    if os.path.exists(pipeAssetDBPath)!=True:
        os.makedirs(pipeAssetDBPath, 7777)
    if os.path.exists(pipeAssetProdPath)!=True:
        os.makedirs(pipeAssetProdPath, 7777)
...
# Update Asset Library XML    
    ...
    if listAssets!= "*N/A*":
        for asset in listAssets:
            ...
            tempNewAssetIDObj = xmlT.SubElement(assetNameObj,asset+"Id")
            tempNewAssetIDObj.text = id
    assetNameObj = xmlT.SubElement(assetLibRoot,newAssetName)
    tempNewAssetIDObj = xmlT.SubElement(assetNameObj,newAssetName+"Id")
    tempNewAssetIDObj.text = newAssetID
...
# Create Asset Based XML File
    xmlPath = pipeAssetLibPath + "assets/" + newAssetID + "/xml/" + newAssetName + ".asset.xml"
...
# Make directories for Asset
    if os.path.exists(pipeAssetLibPath+"assets/"+ newAssetID + "/xml/")!=True:
        os.makedirs(pipeAssetLibPath+"assets/"+ newAssetID + "/xml/", 7777)
    if os.path.exists(pipeAssetLibPath+"assets/"+ newAssetID + "/data/")!=True:
        os.makedirs(pipeAssetLibPath+"assets/"+ newAssetID + "/data/", 7777)
    if os.path.exists(pipeAssetLibPath+"assets/"+ newAssetID + "/data/definition/")!=True:
        os.makedirs(pipeAssetLibPath+"assets/"+ newAssetID + "/data/definition/", 7777)
    if os.path.exists(pipeAssetLibPath+"assets/"+ newAssetID + "/data/variations/")!=True:
        os.makedirs(pipeAssetLibPath+"assets/"+ newAssetID + "/data/variations/", 7777)
    if os.path.exists(pipeAssetLibPath+"assets/"+ newAssetID + "/data/textures/")!=True:
        os.makedirs(pipeAssetLibPath+"assets/"+ newAssetID + "/data/textures/", 7777)
...
# Make directories for variations and representations
    ...
    if tempVars[0] != "*N/A*":
         for tempVar in tempVars:
            os.makedirs(pipeAssetLibPath+"assets/"+ newAssetID + "/data/variations/" + tempVar + "/", 7777)
            tempReps = pipeListAssetVarReps(newAssetType, tempVar)
            for tempRep in tempReps:
                os.makedirs(pipeAssetLibPath+"assets/"+ newAssetID + "/data/variations/" + tempVar + "/" + tempRep, 7777)
# Remove the Lock
...
# Get Info
    ...
    assetXMLPath = pipeBasePath + pipeShow + "/lib/assets/"  + tempID + "/xml/" + assetName + ".asset.xml"
    ...
    if tempVariations[0]!="*N/A*":
            for tempVar in tempVariations:
                ...
                if tempName==assetName and tempCommand=="addRep" and tempVar == input[0]:
                    ...
                    if os.path.exists(pipeAssetLibPath+"assets/"+ tempID + "/data/variations/" + input[0] +"/" + input[1] + "/")!=True:
                        os.makedirs(pipeAssetLibPath+"assets/"+ tempID + "/data/variations/" + input[0] +"/" + input[1] + "/", 7777)
...
# Add Variation
    if tempCommand=="addVar" and tempName==assetName:
            ...
            if os.path.exists(pipeAssetLibPath+"assets/"+ tempID + "/data/variations/"+ tempNewVarName)!=True:
                os.makedirs(pipeAssetLibPath+"assets/"+ tempID + "/data/variations/"+ tempNewVarName, 7777)
                os.makedirs(pipeAssetLibPath+"assets/"+ tempID + "/data/definition/"+ tempNewVarName, 7777)
...
#                print (pipeAssetLibPath+"assets/"+ tempID + "/data/variations/" + tempNewVarName +"/" + tempInput + "/")
                if tempNewVarName != tempInput:
                    ...
                    if os.path.exists(pipeAssetLibPath+"assets/"+ tempID + "/data/variations/" + tempNewVarName +"/" + tempInput + "/" + tempNewSubdir)!=True:
                        os.makedirs(pipeAssetLibPath+"assets/"+ tempID + "/data/variations/" + tempNewVarName +"/" + tempInput + "/" + tempNewSubdir, 7777)
                    if tempTypeTester!="Match":
...
def pipeListAssetVars(assetName):
    ...
    tempXmlPath =  (pipeAssetLibPath + "assets/" + tempID + "/xml/"+assetName + ".asset.xml")
    tempValues = []
    tree = xmlT.ElementTree(file=tempXmlPath)
