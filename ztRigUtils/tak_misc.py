...
def mirJntUi():
    ...
    cmds.textFieldGrp('srchTxtFldGrp', label='Search For: ', columnWidth=[(1, 70), (2, 50)], text='L_')
    cmds.textFieldGrp('rplcTxtFldGrp', label='Replace With: ', columnWidth=[(1, 70), (2, 50)], text='R_')
...
def mirObjUi():
    ...
    cmds.textFieldGrp('srchTxtFldGrp', label='Search for: ', text='L_', columnWidth=[1, 70])
    cmds.textFieldGrp('rplcTxtFldGrp', label='Replace with', text='R_', columnWidth=[1, 70])
...
def mirCtrlShapeUi():
    ...
    cmds.textFieldGrp('srchTxtFld', label='Search: ', text='L_', columnWidth=[(1, 50), (2, 50)])
    cmds.textFieldGrp('rplcTxtFld', label='Replace: ', text='R_', columnWidth=[(1, 50), (2, 50)])
...
def openCWD():
    ...
    filePath = cmds.fileDialog2(fileMode=1, caption='Open', startingDirectory=str(sceneName.dirname()), fileFilter='Maya Files (*.ma *.mb)')
    if filePath:
        cmds.file(filePath[0], open=True, force=True)


def saveCWD():
    sceneName = pm.sceneName()
    filePath = cmds.fileDialog2(fileMode=0, caption='Save as', startingDirectory=str(sceneName.dirname()), fileFilter='Maya Files (*.ma *.mb)')
...
def importCWD():
    sceneName = pm.sceneName()
    filePath = cmds.fileDialog2(fileMode=1, caption='Open', startingDirectory=str(sceneName.dirname()))
    if filePath:
...
def exportCWD():
    sceneName = pm.sceneName()
    filePath = cmds.fileDialog2(fileMode=0, caption='Export', startingDirectory=str(sceneName.dirname()))
    if filePath:
        cmds.file(rename=filePath[0])
...
def centerOfPoints():
    ...
    cmds.parentConstraint(clust,tempLocator,mo=0)
...
def locGrp():
    ...
    for sel in selList:
        ...
        zroGrp = cmds.duplicate(loc, po=True, n=loc + "_space")
...
def doGroup(obj, suffix):
    ...
    pm.xform(grpNode, matrix=objWsMatrix)
...
def editDfmMember(mode, *args):
    ...
    selList = cmds.ls(sl=True)
    if mode == 'add':
        cmds.sets(selList, add=dfmSet)
    if mode == 'remove':
        cmds.sets(selList, remove=dfmSet)
...
def arrangeInARow(offset=1.0):
    ...
    for sel in selList[1:]:
        for channel in cmds.listAttr(sel, keyable=True):
            cmds.setAttr('{}.{}'.format(sel, channel), lock=False)
...
def addInfCopySkin():
    ...
    srcSkinClst = mel.eval('findRelatedSkinCluster("%s");' % srcSkinGeo)
    srcInfs = selInflu()
...
def setupSoftModCtrl(geometry=None):
    ...
    softModCtrl.makeGroup(space=True, extra=True, auto=False)
    softModSlideCtrl = control.Control(name='%s_softMode%dSlide_ctrl' % (geometry.name(), count), shape='circleY')
    softModSlideCtrl.setScale(2)
    softModSlideCtrl.makeGroup(space=True, extra=False, auto=False)
...
def unlockChannels():
    sels = pm.selected()
    for sel in sels:
        for attr in sel.listAttr(keyable=True):
            attr.unlock()
...
def createCrvLocCtrls(crv):
    crv = pm.PyNode(crv) if isinstance(crv, basestring) else crv
