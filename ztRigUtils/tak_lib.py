...
def copySkin(source, target):
    source = pm.PyNode(source) if isinstance(source, basestring) else source
    target = pm.PyNode(target) if isinstance(target, basestring) else target
...
def constraintWithMatrix(driver, driven, maintainOffset=True, translateAxes=['x','y','z'], rotateAxes=['x','y','z'], scaleAxes=[]):
    driver = pm.PyNode(driver) if isinstance(driver, basestring) else driver
    driven = pm.PyNode(driven) if isinstance(driven, basestring) else driven
    multMtx = pm.createNode('multMatrix', n=driver.name()+'_multMtx')
    tempMultMtx = pm.createNode('multMatrix')
    decMtx = pm.createNode('decomposeMatrix', n=driver.name()+'_decMtx')

    if maintainOffset:
        driven.worldMatrix >> tempMultMtx.matrixIn[0]
        driver.worldInverseMatrix >> tempMultMtx.matrixIn[1]
        multMtx.matrixIn[0].set(tempMultMtx.matrixSum.get())
        pm.delete(tempMultMtx)
...
def interpolateRotation(driverA, driverB, driven, driverAw, driverBw):
    driverA = pm.PyNode(driverA) if isinstance(driverA, basestring) else driverA
    driverB = pm.PyNode(driverB) if isinstance(driverB, basestring) else driverB
    driven = pm.PyNode(driven) if isinstance(driven, basestring) else driven
...
def copySDK(drivenObj, searchStr, replaceStr, inverseAttrs=[]):
    drivenObj = pm.PyNode(drivenObj) if isinstance(drivenObj, basestring) else drivenObj

    animNodes = drivenObj.connections(d=False, type='animCurve')
        ...
        if inverseAttrs:
            for invAttr in inverseAttrs:
                if invAttr in outputPlug.name():
                    [dupAnimNode.setValue(id, -dupAnimNode.getValue(id)) for id in xrange(dupAnimNode.numKeys())]
